...
const PotentialCustomerList = (props) => {
	...
	const editPotentialCustomer = () => {
		...
		} else if (values.isProminent === "") {
			setErrorMessage("Please Select Prominent status");
		} else if (values.isProminent === "-1") {
			setErrorMessage("Please Select Prominent status");
		} else if (values.phone.trim() === "") {
			setErrorMessage("Please Select District");
		} else if (values.phone.trim() === "-1") {
			setErrorMessage("Please Select District");
		} else {
			axios
				.put(
					url + "/potential_customer/" + updateID,
					{
						...
						isProminent: values.isProminent,
					},
					...
					>
                        ...
                        /> */}
											...
											<Grid item md={6} xs={12}>
												<TextField
													fullWidth
													label="Select Prominent Type"
													name="isProminent"
													onChange={handleChange}
													required
													select
													error={
														values.isProminent == "-1"
															? values.inputError
																? true
																: false
															: ""
													...
													variant="outlined"
												>
													{prominentType.map((option) => (
														<option
															key={option.value}
															value={option.value}
															selected={
																Number.parseInt(values.isProminent) ===
