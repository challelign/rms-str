...
const ProfileDetails = ({ props, className, ...rest }) => {
  ...
  const handleChange = event => {
    ...
    if (event.target.name == 'response' && event.target.value == 1) {
      setValues({
        ...values,
        disabled: true,
        [event.target.name]: 1
    ...
    } else if (event.target.name == 'response' && event.target.value == 2) {
      setValues({
        ...values,
        disabled: false,
        [event.target.name]: 2
    ...
    <form autoComplete="off" noValidate onSubmit={handleSubmit}>
      {updateID != 0 ? (
        <Card>
          ...
          <CardHeader
            subheader="What was customer's response ? and reason for account being below 500,000?"
            ...
            <Grid container spacing={3}>
              <Grid item md={6} xs={12}>
                <TextField
                  ...
                  required
                  select
                  error={values.response == -1 ?  values.inputError ?   true : false :''}
