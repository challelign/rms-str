...
LoggedUser.create = ( userId, first_name, last_name, position, branch_code, branch, result) => {
...
LoggedUser.getAllLeadingBranch = (page, countPerPage,BOResourceLoggedIn, result) => {
  const pageCount = page * countPerPage;
...
LoggedUser.getAllLeadingUser = (page, countPerPage,BOResourceLoggedIn, result) => {
  const pageCount = page * countPerPage;
  const sql = `SELECT id,branch_code, name, branch_name, (
  SUM( big_withdrowal_count ) + SUM( shareholder_count ) + SUM( top_depositors_count ) + SUM( dormant_account_count ) + SUM( potential_customer_count )
  ) AS Count
...
LoggedUser.update_action = ( big_withdrowal_count, shareholder_count, top_depositors_count,dormant_account_count,potential_customer_count,below_500_count, user_id, result) => {
...
LIMIT 1`;
  db.query(sql, [big_withdrowal_count, shareholder_count , top_depositors_count, dormant_account_count , potential_customer_count, below_500_count, user_id], (err, res) => {
    if (err) {
      result(null, err);
      return;
    }
