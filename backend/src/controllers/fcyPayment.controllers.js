...
exports.findAll = (req, res) => {
	if (req.session.autenticated) {
		...
		if (myBoolean) {
			...
			FcyPayment.getAll(page, countPerPage, BOResourceLoggedIn, (err, data) => {
				if (err)
					res.status(500).send({
						message:
							err.message || "Some error occurred while retrieve customers.",
				...
				if (err) {
					...
					err.result === "not_found"
						? res
								.status(404)
								.send({
									message: `Not found shareholder with id ${deleteCustomerPaymentId}`,
								})
						: res.status(500).send({
								message: `Could not delete shareholder with id ${deleteCustomerPaymentId}`,
						  });
				} else res.send({ message: "shareholder deleted successfully!" });
			}
		);
	} else {
